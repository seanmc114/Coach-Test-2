...
function detectTags(prompt, answer, lang, rubric){
  ...
  if(aTrim && wc < Math.min(4, (rubric?.minWords||6))) tags.push("too_short");
...
function tipForTags(tags, lang){
  ...
  if(lang==="es"){
    ...
    if(has("ok")) return "That scores. Keep that standard.";
...
function jcMarking(promptText, answer, level, lang, rubric, det){
  ...
  // Score: reward intelligible communication first (buzz), then add bonuses, then deduct for real language cogs.
// Target: an understandable, on-topic, mostly-correct line should land around 6–8/10.
let score = 4; // baseline attempt credit

// If it's at least a real sentence attempt, give solid credit
if(wc >= 4) score = 6;

// Reward stretch (but don't make length the whole game)
if(wc >= Math.max(6, rubric.minWords)) score = 7;
if(wc >= Math.max(9, rubric.minWords + 3)) score = 8;

// Checklist coverage bonus (but keep it modest)
...
"${basePrompt||"die Aufgabe"}"`;
      ...
      // (Gym) Do not show a full model sentence.
