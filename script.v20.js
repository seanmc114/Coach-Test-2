...
function presentCoachModal(){
  try{
    ...
    const also = top.filter(t=>t && t.label && t.tag && t.tag!==focusTag && t.tag!=="ok" && t.tag!=="detail" && t.tag!=="good" && t.label!=="Good" && t.label!=="OK").slice(0,3).map(t=>t.label);
...
${prompt ? (()=>{ try{
  ...
  const topic = detectTopicES(prompt);
  const starters = (WORKSHOP_STARTERS_EN[topic]||WORKSHOP_STARTERS_EN.general);
  // Strong hint, not a full answer:
  return `<div class="muted" style="margin-top:8px"><b>Starter:</b> Start with one frame: “${escapeHtml(starters)}”</div>`;
}catch(_){ return ""; } })() : ""}
...
// English filler / off-topic (e.g. "Dublin great") should not be treated as spelling.
if(wc <= 3 && /\b(great|cool|nice|awesome|class|deadly)\b/.test(low)){
  tags.push("off_topic");
  examples.push("Write it in Spanish (use a starter frame: ‘Mi ciudad favorita es…’).");
}

// Agreement quick-check: plural subject + 'son/están' + singular adjective (grande → grandes)
if(/\b(las|los|mis|tus|sus|unas|unos)\s+\w+s\b/.test(low) && /\b(son|estan|están)\b/.test(low)){
  const mAg = low.match(/\b(son|estan|están)\s+([a-záéíóúñ]+)\b/);
  if(mAg){
    const adj = mAg[2]||"";
    if(adj && !adj.endsWith("s") && !adj.endsWith("es")){
      tags.push("agreement");
      examples.push("Plural agreement: add -s (grande → grandes).");
    }
  }
}

// Routine frame error: "mi rutina soy ..." should be "En mi rutina ..." or "Mi rutina es ..."
if(/\b(mi|tu|su)\s+rutina\s+soy\b/.test(low)){
  tags.push("verb_form");
  examples.push("Use the routine frame: ‘En mi rutina…’ (desayuno/estudio…) or ‘Mi rutina es…’");
}
...
state.mark.items = state.mark.items.map(it=>{
  ...
  const reasonMap = {
    ...
    missing_be: "You need the ‘to be’ verb to make descriptions work.",
    no_connector: "Your ideas need linking. One correct connector makes it flow.",
    too_short: "Too short loses marks. Add one detail and you jump up.",
    detail: "Detail wins marks. One extra fact is the difference."
  };
...
// Feedback scoreboard (shown on the feedback page so lads can "win")
try{
  const totalPtsFB = (m.items||[]).reduce((a,it)=>a + (Number(it.score10)||0), 0);
  const pctFB = Math.round((totalPtsFB/(PROMPTS_PER_ROUND*10))*100);
  const sum=document.createElement("div");
  sum.className="fbSummary";
  sum.innerHTML = `
    <div class="fbSummaryInner">
      <b>Total:</b> ${totalPtsFB}/${PROMPTS_PER_ROUND*10} (${pctFB}%)
      <span style="opacity:.75">•</span>
      <b>Time:</b> ${fmtTime(state.elapsedMs)} <span style="opacity:.75">(tie-break)</span>
    </div>`;
  el.feedbackList.appendChild(sum);
      ...
      items.forEach(it=>{
        ...
        card.innerHTML = `
          ...
          <div class="fbMeta">
            <div class="fbScore">${Number.isFinite(Number(it.score10)) ? `${Number(it.score10)}/10` : ""}</div>
...
if((type==="detail" || type==="upgrade") && !(L==="es" && type==="detail")){
